...
// assumption is that the PQ is sorted, based on strcmp
lnode *lnode_PQ_enqueue(lnode **list, char *str)
{
   if (str == NULL) {
      return NULL;
   }
...
lnode *lnode_PQ_dequeue(lnode **list)
{
   return lnode_remove(list);
}
...
int main(int argc, char **argv)
{
   ...
   lnode *new_node = lnode_PQ_enqueue(&head, new_string);

   new_string = (char *)malloc(sizeof(char)*(strlen("efgh")+1));
   strcpy(new_string, "efgh");
   new_node = lnode_PQ_enqueue(&head, new_string);
   ...
   while (head != NULL) {
      lnode *removed_node = lnode_PQ_dequeue(&head);
      printf("%s\n", removed_node->str);
